function y=MQPSO(MaxIter,SwarmSize,ParticleSize,ParticleScope,ObjectiveFunction)
%%
%clear
%clc
%下面定义算法变量
...
for t=1:QPSO.MaxIter
    %%
     QPSO.Beta= (QPSO.MaxIter-t).*sin(rand())/QPSO.MaxIter;
...
%         QPSO.mbest.Loc(1,i)=sum(QPSO.Pbest.Loc(:,i).*c1+c2.*QPSO.Gbest.Value)/QPSO.SwarmSize;
%     end
%     
     for i=1:QPSO.ParticleSize
    ...
    %%
    for i=1:QPSO.SwarmSize
        QPSO.U=betarnd(1,QPSO.ParticleSize);
%         QPSO.Fi=betarnd(1,QPSO.ParticleSize);
%         
         PP=c1*QPSO.Pbest.Loc(i,:)+c2.*QPSO.Gbest.Loc(1,:);
